...
function ExpenseList() {
  ...
  const handleEditSave = (id) => {
    editExpense(id, { ...editForm, amount: parseFloat(editForm.amount) });
    setEditingId(null);
    setEditForm({});
  };
  ...
  if (sortBy === "newest") sortedExpenses.sort((a, b) => new Date(b.date) - new Date(a.date));
  if (sortBy === "oldest") sortedExpenses.sort((a, b) => new Date(a.date) - new Date(b.date));
  if (sortBy === "high") sortedExpenses.sort((a, b) => b.amount - a.amount);
  if (sortBy === "low") sortedExpenses.sort((a, b) => a.amount - b.amount);

  const exportToCSV = () => {
    const headers = ["Description", "Amount", "Category", "Date", "Recurring"];
    const rows = sortedExpenses.map((e) => [
      e.description, e.amount, e.category, e.date, e.recurring ? "Yes" : "No",
    ]);
              ...
              <>
                <div className="expense-item-left">
                  ...
                  <div className="expense-meta-row">
                    ...
                    {expense.recurring && <span className="expense-meta">🔁 Recurring</span>}
                  </div>
                </div>
